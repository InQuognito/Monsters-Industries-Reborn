...
execute if score @s armorLevel matches ..4 unless score #netheriteCount temp matches 128.. run title @s actionbar [{"text":"You do not have enough netherite to purchase ","color":"red"},{"text":"Netherite Armor","color":"dark_gray"},{"text":"!","color":"red"}]
execute if score @s armorLevel matches ..4 if score #paperCount temp matches 1024.. if score #netheriteCount temp matches 128.. run function mi:shop/armor/armor5_buy
