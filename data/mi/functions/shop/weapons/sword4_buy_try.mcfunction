...
execute if score @s swordLevel matches ..3 unless score #netheriteCount temp matches 32.. run title @s actionbar [{"text":"You do not have enough netherite to purchase a ","color":"red"},{"text":"Netherite Sword","color":"dark_gray"},{"text":"!","color":"red"}]
execute if score @s swordLevel matches ..3 if score #paperCount temp matches 512.. if score #netheriteCount temp matches 32.. run function mi:shop/weapons/sword4_buy
