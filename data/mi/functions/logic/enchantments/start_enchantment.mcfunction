...
execute if entity @s[predicate=mi:is_holding_enchantable] unless score #tokenCount temp >= #enchanting price run title @s actionbar {"text":"You do not have enough tokens to enchant!","color":"red"}
execute if entity @s[predicate=mi:is_holding_enchantable] if score #tokenCount temp >= #enchanting price run tag @s add priceApproved
