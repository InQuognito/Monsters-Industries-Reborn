...
execute unless score $currentStock stock matches 350.. run execute unless score #paperCount temp matches 128.. run title @s actionbar {"text":"You do not have enough paper to purchase stock!","color":"red"}

execute unless score $currentStock stock matches 350.. run execute if score #paperCount temp matches 128.. run function mi:logic/interactions/others/buy_stock1